    ...
    public class VisitorService
    {
        ...
        public (int statusCode, string message) CheckIn(int visitId)
        {
            ...
            AND IsActive = 1";
                ...
                DateTime visitDate =
                    Convert.ToDateTime(reader["VisitDate"]);

                bool alreadyCheckedIn =
                    Convert.ToBoolean(reader["CheckIn"]);
...
SELECT
    ...
    vt.CheckOut
FROM Visitors v
INNER JOIN Visits vt
    ON v.VisitorId = vt.VisitorId
WHERE vt.VisitId = @VisitId";

                using SqlCommand auditDataCmd =
                    new SqlCommand(auditDataQuery, con);

                auditDataCmd.Parameters.AddWithValue(
                    "@VisitId",
                    visitId);

                using SqlDataReader auditReader =
                    auditDataCmd.ExecuteReader();
        ...
        public (int statusCode, string message) CheckOut(int visitId)
        {
            ...
            AND IsActive = 1";
                ...
                bool checkedOut =
                    Convert.ToBoolean(reader["CheckOut"]);
        ...
        public (bool success, string? error) CreateVisit(CreateVisitRequest request)
        {
            ...
            AND IsActive = 1";
                ...
                using SqlCommand visitorCmd =
                    new SqlCommand(visitorQuery, con);

                visitorCmd.Parameters.AddWithValue("@VisitorId", request.VisitorId);

                int visitorCount =
                    Convert.ToInt32(visitorCmd.ExecuteScalar());
            ...
            AND IsActive = 1";
                ...
                string status =
                    scheduledCount < 10 ? "Scheduled" : "Waiting";
        ...
        public (List<VisitReportResponse>? visits, string? error)
    GetScheduledVisits(DateTime visitDate)
        {
            try
            {
                string? connectionString =
        ...
        public (List<VisitReportResponse>? visits, string? error)
    GetCompletedVisits(DateTime visitDate)
        {
            try
            {
                string? connectionString =
        ...
        public (int statusCode, string message)
    CancelVisit(int visitId, CancelVisitRequest request)
        {
            try
            {
                string? connectionString =
        ...
        public (int statusCode, string message) ApproveVisit(int visitId)
        {
            ...
            AND IsActive = 1";
                ...
                using SqlCommand visitCmd =
                    new SqlCommand(visitQuery, con);

                visitCmd.Parameters.AddWithValue("@VisitId", visitId);

                using SqlDataReader reader =
                    visitCmd.ExecuteReader();
        ...
        public (List<VisitReportResponse>? visits, string? error)
    GetWaitingVisits(DateTime visitDate)
        {
            try
            {
                string? connectionString =
        ...
        public (List<VisitReportResponse>? visits, string? error)
    GetCancelledVisits(DateTime visitDate)
        {
            try
            {
                string? connectionString =
        ...
        public (DashboardResponse? dashboard, string? error)
    GetDashboard(DateTime visitDate)
        {
            try
            {
                string? connectionString =
        ...
        public (List<VisitorHistoryResponse>? history, string? error)
    ...
    UpdateVisitor(
        ...
        UpdateVisitorRequest request)
        {
            ...
            AND IsActive = 1";
                ...
                using SqlCommand existsCmd =
                    new SqlCommand(existsQuery, con);
                ...
                int exists =
                    Convert.ToInt32(existsCmd.ExecuteScalar());
    ...
    WHERE VisitorId = @VisitorId";
                ...
                using (SqlCommand oldCmd =
                    new SqlCommand(getOldDataQuery, con))
                {
    ...
    (
        ...
        'Admin'
                ...
                using SqlCommand auditCmd =
                    new SqlCommand(auditQuery, con);

                auditCmd.Parameters.AddWithValue("@VisitorId", visitorId);
                auditCmd.Parameters.AddWithValue("@OldName", oldName);
                auditCmd.Parameters.AddWithValue("@OldPhone", oldPhone);
                auditCmd.Parameters.AddWithValue("@OldBirthDate", oldBirthDate);

                auditCmd.Parameters.AddWithValue("@NewName", request.Name);
                auditCmd.Parameters.AddWithValue("@NewPhone", request.PhoneNumber);
                auditCmd.Parameters.AddWithValue("@NewBirthDate", request.BirthDate);
    ...
    DeleteVisitor(int visitorId)
        ...
        public (List<VisitorAuditResponse>? audits, string? error)
    GetVisitorAudit(int visitorId)
        {
            try
            {
                string? connectionString =
